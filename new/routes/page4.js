...
router.get('/page43/:id',isLoggedIn, (req, res)=>{
    connection.query(queries.lists(req.params.id)+ '; '+ queries.user, (err, result)=>{
        if(err){
            console.log(err)
        }else{
            console.log(result[1])
            res.render('page4/page43',{question: result[0] , list: result[1] , title:req.params.id});
